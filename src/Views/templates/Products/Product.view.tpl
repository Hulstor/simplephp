...
<section class="container-m row px-4 py-4">
  ...
  <form action="index.php?page=Products_Product&mode={{~mode}}&productId={{productId}}" method="POST" class="col-12 col-m-8 offset-m-2">
    <div class="row my-2 align-center">
      ...
      <input class="col-12 col-m-9" readonly disabled type="text" id="productIdD" placeholder="Código" value="{{productId}}" />
    ...
    <div class="row my-2 align-center">
      ...
      {{if productName_error}}<div class="col-12 col-m-9 offset-m-3 error">{{productName_error}}</div>{{endif productName_error}}
    </div>
    ...
    <div class="row my-2 align-center">
      ...
      {{if productDescription_error}}<div class="col-12 col-m-9 offset-m-3 error">{{productDescription_error}}</div>{{endif productDescription_error}}
    </div>

    <div class="row my-2 align-center">
      <label class="col-12 col-m-3" for="productPrice">Precio</label>
      <input class="col-12 col-m-9" {{~readonly}} type="number" name="productPrice" id="productPrice" placeholder="0.00" min="0" step="0.01" value="{{productPrice}}" />
      {{if productPrice_error}}<div class="col-12 col-m-9 offset-m-3 error">{{productPrice_error}}</div>{{endif productPrice_error}}
    </div>

    <div class="row my-2 align-center">
      <label class="col-12 col-m-3" for="productImgUrl">Url de Imagen (250 x 200)</label>
      <input class="col-12 col-m-9" {{~readonly}} type="text" name="productImgUrl" id="productImgUrl" placeholder="https://..." value="{{productImgUrl}}" />
      {{if productImgUrl_error}}<div class="col-12 col-m-9 offset-m-3 error">{{productImgUrl_error}}</div>{{endif productImgUrl_error}}
    </div>
    ...
    <div class="row my-2 align-center">
      ...
      {{if ~readonly}}<input type="hidden" name="productStatus" value="{{productStatus}}" />{{endif ~readonly}}
    </div>
  {{endwith product}}

    <div class="row my-4 align-center flex-end">
      {{if showCommitBtn}}<button class="primary col-12 col-m-2" type="submit" name="btnConfirmar">Confirmar</button>&nbsp;{{endif showCommitBtn}}
