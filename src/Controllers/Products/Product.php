...
class Product extends PublicController
{
    ...
    private function getData()
    {
        ...
        if ($this->mode !== "INS") {
            ...
            if ($productId <= 0) {
                throw new \Exception("Parámetro productId inválido", 1);
            }
        ...
        if (!$this->isPostBack()) {
            $this->product_xss_token = md5(time() . rand());
            $_SESSION["product_xss_token"] = $this->product_xss_token;
        }
    }
    ...
    private function validateData()
    {
        ...
        if (!isset($_SESSION["product_xss_token"]) || $_SESSION["product_xss_token"] !== $this->product_xss_token) {
            throw new \Exception("Token de seguridad inválido", 1);
        }
        ...
        if (Validators::IsEmpty($this->product["productPrice"]) || $this->product["productPrice"] <= 0) {
            $errors["productPrice_error"] = "El precio es requerido y debe ser mayor a cero";
        }
